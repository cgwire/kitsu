...
export default {
  ...
  computed: {
    ...
    availableTaskTypes () {
      const taskTypes = sortByName(this.taskTypes.filter(taskType => {
        return this.form.task_types.indexOf(taskType.id) === -1 && taskType.for_entity === 'Asset'
      }))
      return [
        {
          name: '+ Task Type',
    ...
    assetTypeToEdit () {
      if (this.assetTypeToEdit.id) {
        this.form = {
          ...
          task_types: [...this.assetTypeToEdit.task_types]
        }
      } else {
