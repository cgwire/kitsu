          ...
          <form class="mt1" @submit.prevent="saveAssignments()">
            ...
            <div class="flexrow mt2">
              ...
              <text-field
                ...
                required
                type="number"
                v-model="assignments.dailyQuotas"
              />
            </div>
            <div class="mt2">
              <div>{{ $t('schedule.quotas_title') }}</div>
              <div>{{ $t('schedule.quotas_seconds') }} {{ 0 }}</div>
              <div>{{ $t('schedule.quotas_counts') }} {{ 0 }}</div>
            </div>
...
export default {
  ...
  methods: {
    ...
    onScheduleItemDropped(event, item) {
      ...
      this.assignments.startDate = event.start_date || item.start_date
      ...
      for (const taskType of this.draggedEntities) {
        ...
        for (const entity of taskType.children) {
          ...
          const taskEstimation =
            this.assignments.dailyQuotas > 0 ? this.assignments.dailyQuotas : 1
          ...
          while (nextAssigneeIndex < this.availablePersons.length) {
            ...
            if (taskEndDate.isAfter(endDate)) {
              ...
              // set start & end dates
              const daysLength =
                this.assignments.dailyQuotas ||
                getBusinessDays(taskStartDate, taskEndDate)
