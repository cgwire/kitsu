...
class FileStatusTestCase(ApiDBTestCase):
    ...
    def test_get_file_statuss(self):
        file_statuss = self.get("data/file_status")
        self.assertEquals(len(file_statuss), 3)

    def test_get_file_status(self):
        file_status = self.get_first("data/file_status")
        file_status_again = self.get("data/file_status/%s" % file_status["id"])
        self.assertEquals(file_status, file_status_again)
        self.get_404("data/file_status/%s" % fields.gen_uuid())
    ...
    def test_create_file_status(self):
        ...
        self.file_status = self.post("data/file_status", data)
    ...
    def test_update_file_status(self):
        ...
        self.put("data/file_status/%s" % file_status["id"], data)
        file_status_again = self.get(
            "data/file_status/%s" % file_status["id"])
        self.assertEquals(data["color"], file_status_again["color"])
        self.put_404("data/file_status/%s" % fields.gen_uuid(), data)

    def test_delete_file_status(self):
        file_statuss = self.get("data/file_status")
        self.assertEquals(len(file_statuss), 3)
        file_status = file_statuss[0]
        self.delete("data/file_status/%s" % file_status["id"])
        file_statuss = self.get("data/file_status")
        self.assertEquals(len(file_statuss), 2)
        self.delete_404("data/file_status/%s" % fields.gen_uuid())
